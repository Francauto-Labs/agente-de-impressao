 ...
 *   PRINT_MODE: raw | normalize | pcl | ps   (padrão: pcl)
 *   PRINT_COLOR: "true" para colorida
 *   GHOSTSCRIPT_BIN: padrão "gs"
 *   SMB_USER / SMB_PASSWORD / SMB_DOMAIN: credenciais para filas \\host\fila
 * ===========================================================================*/
type PrintMode = "raw" | "normalize" | "pcl" | "ps";

const PRINT_MODE = (process.env.PRINT_MODE ?? "pcl") as PrintMode;
...
const SMB_TIMEOUT_MS = 60_000;

// Fila FIFO por destino: evita conexões concorrentes na mesma impressora.
const printerQueues: Map<string, Promise<void>> = new Map();

/**
 * Envia um arquivo para impressão.
 * @param fileUrl   URL http(s) do PDF (ou caminho local).
 * @param printerIp Destino: IP puro ("211.2.100.193") OU share ("\\\\host\\fila" | "//host/fila").
 */
export async function sendToPrinter(
  ...
  // roda depois do job anterior do mesmo destino, deu certo ou não
  const run = prev.then(
    () => executePrintJob(fileUrl, printerIp),
    () => executePrintJob(fileUrl, printerIp)
  );

  // "tracked" é o que fica no Map e nunca rejeita (senão a fila trava)
...
function parseShare(dest: string): { service: string } {
...
async function executePrintJob(
  ...
  try {
    const pdfPath = await materializePdf(fileUrl, DOWNLOAD_TIMEOUT_MS);
    tmpFiles.push(pdfPath);

    // Converte para PCL/PS (exceto no modo "raw"): elimina o erro de PDF Direct Print
    let toSend = pdfPath;
    if (PRINT_MODE !== "raw") {
      const out = await runGhostscript(
        GS_BIN,
        pdfPath,
        PRINT_MODE,
        PRINT_COLOR,
    ...
    if (isShare(destination)) {
      ...
      await printViaSocket(await fsp.readFile(toSend), destination, SOCKET_TIMEOUT_MS);
    }
    return { success: true };
  } catch (error: any) {
...
/* ------------------------ Destino SHARE: smbclient (Linux) ------------------------ */
function printViaSmb(filePath: string, destination: string): Promise<void> {
  const { service } = parseShare(destination);
  if (!SMB_USER) {
    return Promise.reject(
      new Error(
        `Impressão em share "${destination}" exige SMB_USER/SMB_PASSWORD no ambiente do agent.`
      )
    );
  }
  ...
  return new Promise<void>((resolve, reject) => {
    // senha via env PASSWD (não aparece em `ps`)
    const proc = spawn("smbclient", args, {
      env: { ...process.env, PASSWD: SMB_PASSWORD },
    });
    let stderr = "";
    const timer = setTimeout(() => {
      proc.kill("SIGKILL");
      reject(new Error(`Timeout de impressão (${SMB_TIMEOUT_MS}ms) em ${service}`));
    }, SMB_TIMEOUT_MS);

    proc.stderr.on("data", (d) => (stderr += d.toString()));
    proc.on("error", (err) =>
      reject(
        new Error(`Falha ao executar smbclient: ${err.message}. Instale: apk add samba-client`)
      )
    );
    proc.on("close", (code) => {
      clearTimeout(timer);
      if (code === 0) {
        ...
        reject(new Error(`smbclient saiu com código ${code}: ${stderr}`));
      }
    });
  });
}

/* --------------------------- Destino IP: socket RAW 9100 -------------------------- */
function printViaSocket(
  payload: Buffer,
  destination: string,
  timeoutMs: number
): Promise<void> {
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    socket.connect(port, ip, () => {
      ...
      socket.end(payload); // escreve tudo + FIN, sem destruir à força
    });
    socket.on("error", (err) => {
      hadError = true;
      fail(err);
...
function runGhostscript(
  ...
  const outPath = path.join(
    tmpdir(),
    `print-${randomUUID()}.${mode === "normalize" ? "pdf" : "prn"}`
  );
  const device =
    mode === "normalize"
      ? "pdfwrite"
  ...
  if (mode === "normalize") {
    args.push("-dCompatibilityLevel=1.4", "-dPDFSETTINGS=/prepress");
  } else {
    args.push("-r600");
  }
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    gs.on("error", (err) => {
      ...
      reject(
        new Error(
          `Falha ao executar Ghostscript ("${bin}"): ${err.message}. Instale: apk add ghostscript`
        )
      );
    });
    gs.on("close", (code) => {
      ...
      else reject(new Error(`Ghostscript saiu com código ${code}: ${stderr}`));
    });
  });
}
